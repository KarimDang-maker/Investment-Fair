...
export default function PorteursPage() {
  const { t } = useLanguage();
      ...
      <HeroPorteurs>
        <div className="max-w-5xl mx-auto text-center">
          ...
          >
            ...
            <span className="font-semibold">{t('hero.porteurs.badge')}</span>
          </motion.div>
          ...
          >
            {t('hero.porteurs.title').split(' ')[0]} <span className="underline decoration-yellow-300 decoration-4">{t('hero.porteurs.title').split(' ').slice(1).join(' ')}</span>
          </motion.h1>
          ...
          >
            <Link to="/inscription?type=porteur">
              <Button size="lg" className="bg-white text-emerald-600 hover:bg-emerald-50 hover:scale-105 transition-all duration-300 shadow-2xl group">
      ...
      <SectionContainer className="-mt-8">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-extrabold mb-4">
            {t('porteurs.why.title').split(' ')[0]} <GradientText variant="primary">{t('porteurs.why.title').split(' ').slice(1).join(' ')}</GradientText>
          </h2>
          <p className="text-lg md:text-xl text-gray-600 max-w-2xl mx-auto">
            {t('porteurs.why.subtitle')}
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          <AnimatedCard variant="emerald" delay={0} className="group">
            ...
            <h3 className="text-2xl font-bold mb-4 text-gray-900 group-hover:text-emerald-600 transition-colors">
              {t('porteurs.access.title')}
            </h3>
            <p className="text-gray-600 leading-relaxed">
              {t('porteurs.access.desc')}
            </p>
          </AnimatedCard>
          ...
          <AnimatedCard variant="blue" delay={0.1} className="group">
            ...
            <h3 className="text-2xl font-bold mb-4 text-gray-900 group-hover:text-blue-600 transition-colors">
              {t('porteurs.networking.title')}
            </h3>
            <p className="text-gray-600 leading-relaxed">
              {t('porteurs.networking.desc')}
            </p>
          </AnimatedCard>
          ...
          <AnimatedCard variant="emerald" delay={0.2} className="group">
            ...
            <h3 className="text-2xl font-bold mb-4 text-gray-900 group-hover:text-emerald-600 transition-colors">
              {t('porteurs.visibility.title')}
            </h3>
            <p className="text-gray-600 leading-relaxed">
              {t('porteurs.visibility.desc')}
            </p>
          </AnimatedCard>
          ...
          <AnimatedCard variant="blue" delay={0.3} className="group">
            ...
            <h3 className="text-2xl font-bold mb-4 text-gray-900 group-hover:text-blue-600 transition-colors">
              {t('porteurs.mentoring.title')}
            </h3>
            <p className="text-gray-600 leading-relaxed">
              {t('porteurs.mentoring.desc')}
            </p>
          </AnimatedCard>
        </div>
      </SectionContainer>
      ...
      <SectionContainer variant="gray">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-extrabold mb-4">
            {t('porteurs.benefits.title').split(' ').slice(0, 3).join(' ')} <GradientText variant="primary">{t('porteurs.benefits.title').split(' ').slice(3).join(' ')}</GradientText>
          </h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            {t('porteurs.benefits.subtitle')}
          </p>
        </motion.div>
      ...
      <SectionContainer className="relative overflow-hidden">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-100 rounded-full mb-6">
            ...
            <span className="text-sm font-semibold text-emerald-900">{t('cta.limited')}</span>
          </div>

          <h2 className="text-3xl md:text-5xl font-extrabold mb-6">
            {t('porteurs.cta.title').split(' ').slice(0, 5).join(' ')} <GradientText variant="primary">{t('porteurs.cta.title').split(' ').slice(5).join(' ')}</GradientText>
          </h2>

          <p className="text-xl md:text-2xl text-gray-600 mb-10 font-light">
            {t('porteurs.cta.subtitle')}
          </p>

          <Link to="/inscription?type=porteur">
            <Button size="lg" className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white hover:shadow-2xl hover:scale-105 transition-all duration-300 group">
              <Rocket className="mr-2 h-5 w-5 group-hover:-rotate-12 transition-transform" />
              {t('porteurs.cta.button')}
              <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </Link>

          <p className="text-sm text-gray-500 mt-6">
            {t('porteurs.cta.features')}
          </p>
        </motion.div>
      </SectionContainer>
    </div>
